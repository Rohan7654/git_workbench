...
GitAssist is a powerful, professional CLI suite designed to enhance your Git and GitHub workflow. It provides a set of interactive tools to simplify common tasks, enforce conventions, and visualize repository data.

## Features

GitAssist includes a comprehensive suite of tools:
...
```bash
git clone https://github.com/Rohan7654/git_assist.git
cd git_assist
pip install -e .
```
...
Once installed, you can use the `ga` command (or `git_assist`) to access the suite.
...
See what GitAssist looks like in action.

### 1. Main Menu
Running `ga` launches the central hub:
...
### 2. Git Stats
`ga stats` gives you a comprehensive overview:
...
### 3. Git Undo
`ga undo` helps you recover from mistakes safely:
...
### 4. PR Maker
`ga pr` generates descriptions from your work:
...
### 5. Git Clean
`ga clean` keeps your repository tidy:
...
### 6. Git Basics
`ga basics` is your interactive guide:
...
### 7. Git Who
`ga who` reveals the top contributors:
...
### 8. Commit Lint
`ga lint` enforces your commit standards:
